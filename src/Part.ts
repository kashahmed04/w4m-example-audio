export class Part {
  ...
  constructor(vocalRange: string, audioElementId: string, audioContext: AudioContext) {
    ...
    //have to call things from the constructor so each instance knows what to do and it does not go from top to bottom**
  }
  ...
  expandMarkup(): void {
    ...
    this.markup.appendChild(h2) //this is where we see the words show up for the specfic audio**
  }
  ...
  patch(): MediaElementAudioSourceNode {
    ...
    //and we define this in the constructor its like the**
  ...
  timeTo(timeInSeconds: number): void {
    ...
    //and this is for each slider move according to the main slider
